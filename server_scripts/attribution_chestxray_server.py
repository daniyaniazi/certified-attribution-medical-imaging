#!/usr/bin/env python
"""
ChestXray Attribution Generation Script for Server Execution

Generates attributions (saliency maps) for trained ChestXray models using multiple
attribution methods and saves results for visualization and analysis.

Usage:
    python attribution_chestxray_server.py --num_samples 10
    python attribution_chestxray_server.py --models resnet18 resnet50 --num_samples 20
    python attribution_chestxray_server.py --methods IntegratedGradients GradCAM --no-viz
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Generate attributions for ChestXray models on server')
    ...
    parser.add_argument('--data_root', type=str, default='data/raw/chestxray',
                       help='Path to ChestXray dataset (default: data/raw/chestxray)')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='outputs/checkpoints/chestxray',
                       help='Directory with trained model checkpoints (default: outputs/checkpoints/chestxray)')
    
    # Output paths
    parser.add_argument('--output_dir', type=str, default='outputs/attributions_viz/chestxray',
                       help='Directory to save attributions (default: outputs/attributions_viz/chestxray)')
    parser.add_argument('--log_dir', type=str, default='outputs/logs/attribution_chestxray',
                       help='Directory to save logs (default: outputs/logs/attribution_chestxray)')
...
def load_dataset(data_root, img_size, num_samples):
    """Load ChestXray validation dataset."""
    print('\nLoading ChestXray validation dataset...')
...
def process_models(available_models, args, device, val_dataset, class_names):
    ...
    for model_info in tqdm(models_to_process, desc='Models'):
        ...
        # Load model
        try:
            model, cfg = get_model(model_name, num_classes=2, 

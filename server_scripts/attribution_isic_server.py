#!/usr/bin/env python
"""
ISIC Attribution Generation Script for Server Execution

Generates attributions (saliency maps) for trained ISIC models using multiple
attribution methods and saves results for visualization and analysis.

Usage:
    python attribution_isic_server.py --num_samples 50
    python attribution_isic_server.py --models resnet18 resnet50 --num_samples 100
    python attribution_isic_server.py --methods IntegratedGradients GradCAM --no-viz
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Generate attributions for ISIC models on server')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='outputs/checkpoints/isic',
                       help='Directory with trained model checkpoints (default: outputs/checkpoints/isic)')
    
    # Output paths
    parser.add_argument('--output_dir', type=str, default='outputs/attributions_viz/isic',
                       help='Directory to save attributions (default: outputs/attributions_viz/isic)')
    parser.add_argument('--log_dir', type=str, default='outputs/logs/attribution_isic',
                       help='Directory to save logs (default: outputs/logs/attribution_isic)')
...
def load_dataset(data_root, img_size, num_samples):
    ...
    print('\nLoading ISIC validation dataset...')
...
def process_models(available_models, args, device, val_dataset, class_names):
    ...
    for model_info in tqdm(models_to_process, desc='Models'):
        ...
        # Load model
        try:
            model, cfg = get_model(model_name, num_classes=8, 

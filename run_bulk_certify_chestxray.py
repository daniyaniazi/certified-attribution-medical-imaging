#!/usr/bin/env python
"""Bulk certification runner for resnet18 on ChestXray dataset.
Configurable via environment variables and command-line args.
Outputs: outputs/bulk_certifcation/chestxray/resnet18/
"""
...
def get_default_paths():
    ...
    return {
        ...
        "output_base": project_root / "outputs" / "bulk_certifcation" / "chestxray" / "resnet18",
    }

def parse_args():
    p = argparse.ArgumentParser(description="Bulk certification for ChestXray + resnet18")
...
def run_certification(args):
    ...
    print(f"===== ChestXray: resnet18 certification ({args.num_images} images) =====")
    print(f"Command: {' '.join(cmd)}")
    subprocess.run(cmd, check=True)
    print("✓ ChestXray certification complete.")

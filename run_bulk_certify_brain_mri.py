#!/usr/bin/env python
"""Bulk certification runner for resnet18 on Brain MRI dataset.
Configurable via environment variables and command-line args.
Outputs: outputs/bulk_certifcation/brain_mri/resnet18/
"""
...
def get_default_paths():
    ...
    return {
        ...
        "output_base": project_root / "outputs" / "bulk_certifcation" / "brain_mri" / "resnet18",
    }

def parse_args():
    p = argparse.ArgumentParser(description="Bulk certification for Brain MRI + resnet18")
...
def run_certification(args):
    ...
    print(f"===== Brain MRI: resnet18 certification ({args.num_images} images) =====")
    print(f"Command: {' '.join(cmd)}")
    subprocess.run(cmd, check=True)
    print("✓ Brain MRI certification complete.")

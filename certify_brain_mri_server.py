#!/usr/bin/env python
"""Brain MRI Certification Server Script - Certified Randomized Smoothing for Attributions"""
...
from src.datasets.base import BrainMRIDataset
...
def parse_args():
    p = argparse.ArgumentParser("Brain MRI Certification Server")
    p.add_argument("--checkpoint_dir", type=str, default=str(ROOT / "outputs" / "checkpoints" / "brain_mri"))
    p.add_argument("--data_root", type=str, default=str(ROOT / "data" / "raw" / "brain_mri"))
    p.add_argument("--split", type=str, default="val")
    p.add_argument("--certify_dir", type=str, default=str(ROOT / "outputs" / "certifications" / "brain_mri"))
    p.add_argument("--heatmap_dir", type=str, default=str(ROOT / "outputs" / "certifications" / "brain_mri" / "certified_maps"))
...
def main():
    ...
    dataset = BrainMRIDataset(data_root, split=args.split, transform=val_transform)
    ...
    for model_info in available:
        ...
        try:
            ...
            model, _ = get_model(model_name, num_classes=4, pretrained=use_pretrained, device=device)
    ...
    print("\n✅ Brain MRI certification server job complete")

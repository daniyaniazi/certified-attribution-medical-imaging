#!/usr/bin/env python
"""Fundus Certification Server Script - Certified Randomized Smoothing for Attributions"""
...
def parse_args():
    p = argparse.ArgumentParser("Fundus Certification Server")
    p.add_argument("--checkpoint_dir", type=str, default=str(ROOT / "outputs" / "checkpoints" / "fundus"))
    p.add_argument("--data_root", type=str, default=str(ROOT / "data" / "raw" / "fundus"))
    p.add_argument("--split", type=str, default="val")
    p.add_argument("--certify_dir", type=str, default=str(ROOT / "outputs" / "certifications" / "fundus"))
    p.add_argument("--heatmap_dir", type=str, default=str(ROOT / "outputs" / "certifications" / "fundus" / "certified_maps"))
...
def main():
    ...
    dataset = FundusDataset(data_root, split=args.split, transform=val_transform)
    ...
    for model_info in available:
        ...
        try:
            ...
            model, _ = get_model(model_name, num_classes=5, pretrained=use_pretrained, device=device)
    ...
    print("\n✅ Fundus certification server job complete")

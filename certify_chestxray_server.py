#!/usr/bin/env python
"""ChestXray Certification Server Script - Certified Randomized Smoothing for Attributions"""
...
def parse_args():
    p = argparse.ArgumentParser("ChestXray Certification Server")
    p.add_argument("--checkpoint_dir", type=str, default=str(ROOT / "outputs" / "checkpoints" / "chestxray"))
    p.add_argument("--data_root", type=str, default=str(ROOT / "data" / "raw" / "chestxray"))
    p.add_argument("--split", type=str, default="val")
    p.add_argument("--certify_dir", type=str, default=str(ROOT / "outputs" / "certifications" / "chestxray"))
    p.add_argument("--heatmap_dir", type=str, default=str(ROOT / "outputs" / "certifications" / "chestxray" / "certified_maps"))
...
def main():
    ...
    dataset = ChestXrayDataset(data_root, split=args.split, transform=val_transform)
    ...
    for model_info in available:
        ...
        try:
            ...
            model, _ = get_model(model_name, num_classes=2, pretrained=use_pretrained, device=device)
    ...
    print("\n✅ ChestXray certification server job complete")

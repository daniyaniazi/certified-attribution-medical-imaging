#!/usr/bin/env python
"""
Fundus Multi-Model Training Script for Server Execution

Trains multiple model architectures on the fundus photography dataset (APTOS)
and generates comprehensive evaluation reports and visualizations.

Usage:
    python train_fundus_server.py --epochs 50 --batch_size 32
    python train_fundus_server.py --models resnet18 resnet50 --epochs 100
    python train_fundus_server.py --no-eval  # Skip evaluation after training
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train Fundus models on server')
    ...
    # Data paths
    parser.add_argument('--data_root', type=str, default='data/raw/fundus',
                       help='Path to fundus dataset (default: data/raw/fundus)')
    
    # Output paths
    parser.add_argument('--checkpoint_dir', type=str, default='outputs/checkpoints/fundus',
                       help='Directory to save checkpoints (default: outputs/checkpoints/fundus)')
    parser.add_argument('--log_dir', type=str, default='outputs/logs/fundus',
                       help='Directory to save logs (default: outputs/logs/fundus)')
    parser.add_argument('--metrics_dir', type=str, default='outputs/metrics/fundus',
                       help='Directory to save metrics (default: outputs/metrics/fundus)')
...
def infer_num_classes(dataset):
    ...
    try:
        ...
        return 5  # Fundus default (5 DR severity levels: 0-4)
...
def plot_training_curves(models, log_dir, reports_dir):
    ...
    plt.tight_layout()
    save_path = reports_dir / 'fundus_training_curves.png'
...
def plot_model_comparison(eval_results, best_model_name, reports_dir):
    ...
    plt.title('Model Comparison - Validation Accuracy (Fundus)', fontsize=14, fontweight='bold')
    plt.xticks(range(len(model_names)), model_names, rotation=45, ha='right')
    plt.ylim(0, 100)
    plt.grid(axis='y', alpha=0.3, linestyle='--')
    plt.tight_layout()
    
    save_path = reports_dir / 'fundus_model_comparison.png'
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_confusion_matrix(result, model_name, class_names, reports_dir):
    ...
    plt.title(f'Confusion Matrix - {model_name} (Fundus)', fontsize=14, fontweight='bold')
    ...
    save_path = reports_dir / 'fundus_confusion_matrix.png'
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def save_classification_report(result, model_name, class_names, reports_dir):
    ...
    report_path = reports_dir / 'fundus_classification_report.txt'
    with open(report_path, 'w') as f:
        f.write(f"Classification Report - {model_name} (Fundus)\n")
        f.write("="*80 + "\n\n")
        f.write(report)
...
def main():
    ...
    print('\n' + '='*80)
    print('FUNDUS MULTI-MODEL TRAINING')
    print('='*80)

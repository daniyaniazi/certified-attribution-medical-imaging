...
Deep neural networks achieve high accuracy in medical imaging, yet their decisions are often opaque. Post-hoc attribution methods provide heatmaps that highlight pixels influencing a prediction, but these explanations can be unstable. A small, imperceptible perturbation to the input can drastically change the attribution map while the predicted class stays the same. This undermines trust and complicates method comparison. The paper \emph{Pixel-level Certified Explanations via Randomized Smoothing} proposes a certification framework that delivers per-pixel robustness guarantees for any black-box attribution method \cite{anani2025pixel}. Instead of returning a single image-level robustness score, it outputs certified pixel-wise labels: robustly important, robustly unimportant, or abstain.

\section{Background}
\subsection{Post-hoc Attribution Methods}
Attribution methods map an input image to a heatmap of pixel importance with respect to a predicted class. Three broad families dominate the literature: \textit{gradient-based} methods (e.g., saliency, Guided Backpropagation, Integrated Gradients) \cite{simonyan2014saliency,springenberg2014guided,sundararajan2017integrated}; \textit{activation-based} methods (e.g., Grad-CAM variants) \cite{selvaraju2017gradcam}; and \textit{perturbation-based} methods (e.g., Occlusion, RISE) \cite{zeiler2014occlusion,petsiuk2018rise}. These methods are widely used because they are fast and easy to apply to trained models.

\subsection{Limitations of Post-hoc Attribution Methods}
Attribution maps can change substantially under small input perturbations even when the prediction is unchanged \cite{ghorbani2019fragile,dombrowski2019explanations}. Explanations can also be manipulated or violate implementation invariance \cite{subramanya2019fooling,kindermans2019reliability}, so similar inputs may yield very different heatmaps. These weaknesses also affect model-agnostic explainers \cite{kuppa2020blackbox,carmichael2023unfooling}.
...
Empirical robustness methods regularize or smooth attributions to reduce sensitivity, but provide no formal guarantees \cite{chen2019robust,dombrowski2022towards,wang2020smoothed}. Certified approaches give image-level guarantees on attribution stability via randomized smoothing or related bounds \cite{levine2019certifiably,liu2022renyi,wang2023practical,wang2024certified}, but they do not identify which pixels are robust. Segmentation smoothing certifies structured outputs per pixel \cite{fischer2021scalable}; this motivates reformulating attribution as a segmentation task to obtain pixel-wise guarantees.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{report_img/sparcification.png}
...
\subsection{Outcome}
Qualitative maps show smaller $K$ gives finer details, larger $K$ gives broader regions. Figure \ref{fig:paper_fig5} shows \% certified decreases as $R$ increases. Figure \ref{fig:paper_fig6} shows Certified GridPG improves when certified pixels concentrate in the correct cell. Figure \ref{fig:paper_fig8} shows deletion sharply reduces confidence for LRP/RISE, matching their robustness and localization.

...
This metric checks localization on grid datasets (images split into $m\times m$ cells), based on GridPG \cite{bohle2021convolutional}. Suppose the correct cell is $c$ (in this case, the ground-truth cell is fixed at cell 0). Let $x_i$ be the pixels in cell $i$, and define $A_c(p)=1$ if pixel $p$ is certified important (label 1), otherwise $0$. The certified localization score is
